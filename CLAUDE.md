...
- **Secrets via vault, aldrig disk.** `EMAILDAWG_PASSPHRASE` krypterer de gemte email-credentials. I prod renderes secrets fra homelab Infisical (nipogi-vault) via stack-scoped machine-identity til `/opt/emaildawg/.env` (BWS retireret 2026-07-20). Passphrase/graph_state-nøglen SKAL pinnes i vaulten, ellers churner Graph-subscriptionen ved hver rotation.
